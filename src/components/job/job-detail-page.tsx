...
import type { FeedItem, JobDetailState } from "@/types";
...
export function JobDetailPage() {
  ...
  const item = (location.state as JobDetailState| undefined)
    ?.item as FeedItem | undefined;

  // Not found state
  if (!item || item.job.id !== jobId) {
    return (
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={label} className="flex items-center justify-between text-sm">
                    <span className="text-[#353535]/70 dark:text-[#d9d9d9]/70">{label}</span>
                    <span className="font-medium text-[#353535] dark:text-white">{value}%</span>
                  </div>
                ))}
              </div>
              <div className="mt-3 border-t border-[#d9d9d9]/50 pt-3 dark:border-[#353535]">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-[#353535] dark:text-white">Overall</span>
                  <span className="text-lg font-bold text-[#3c6e71]">{details.overall}%</span>
                </div>
              </div>
            </div>
          )}
